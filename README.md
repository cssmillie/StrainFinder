...
Strain Finder uses the EM algorithm to perform the optimization. Because EM only converges to a local optimum, but not necessarily a global optimum, you need to run Strain Finder with many initial conditions and select the estimate with the best likelihood score. Additionally, because the number of strains is not known in advance, you need to run it for N=2,3,...,k strains. You can then find the optimal number of strains with model selection criteria, such as AIC/BIC scores or the LRT.
